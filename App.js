...
export default class App extends React.Component {
  ...
  //counter function to limit to 5 click
  countClick = () =>{
    if(this.state.count === 5){
      this.setState({winMessage:"Better luck next time.",isDisabled: true});
    }
    if(this.state.lucky === true){
  ...
  scratchItem = itemNumber => {
    if(this.state.randomNumber === itemNumber) {
      ...
      this.setState({winMessage: "Hurray!!! You won.",lucky: true}, () => {this.countClick()});
      this.forceUpdate();
    }else{
      this.countClick();
  ...
  resetGame = () => {
    this.setState({ winMessage: "",randomNumber: this.generateRandomNumber(), isDisabled: false, count:1},
    () => {
  ...
  render(){
    return (
      <View style={styles.container}>
        ...
        <Text style= {styles.message}>{this.state.winMessage}</Text>
        <Button 
        ...
        full promary style={styles.button}>
